...
Thanks for your interest in improving Claude Code Proxy. Issues and pull requests are welcome.
...
- `clients/` — helper scripts for end users (e.g. the Claude Code status line).
...
For production changes, use `scripts/blue-green.sh` as documented in the README.
It requires one existing Traefik gateway and switches only after healthchecks;
`scripts/blue-green.sh status` is safe to run without changing containers.
