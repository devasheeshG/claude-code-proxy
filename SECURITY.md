...
[private vulnerability report](https://github.com/devasheeshG/claude-code-proxy/security/advisories/new)
...
This service brokers inference on a pool of real Claude subscription accounts, so a few secrets are the crown jewels:

- **Pooled-account OAuth tokens** — stored Fernet-encrypted at rest (`access_token_enc` / `refresh_token_enc`). A refresh token grants ongoing access to that Claude account.
