# Quota refresh worker

This note documents the quota refresh worker behavior in the Claude Code Proxy. It is intentionally small so operators and contributors can find one decision without searching the entire README.
...
Run the focused backend test for this area, then the full Claude Code backend suite and frontend checks before deployment. During a rollout, verify both the health endpoint and the promoted slot.
