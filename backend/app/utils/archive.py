...
The archive middleware records only the Claude Messages inference endpoints. It never
...
class ArchiveSession:
    ...
    def start_response(self, status_code: int, content_type: str) -> None:
        ...
        # Keep one stable response name; stored bytes remain exact, including SSE.
    ...
    async def finish(self, *, complete: bool, termination_reason: Optional[str], metadata: Mapping[str, object]) -> None:
        ...
        if self.response_status is not None and self.response_body:
            self.response = await self.store.put_body(f"{self.base_key}/response.json", bytes(self.response_body), self.response_content_type)
        self.finalized = True
