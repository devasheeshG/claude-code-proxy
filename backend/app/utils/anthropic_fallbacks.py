# Path: app/utils/anthropic_fallbacks.py
# Description: Selection, health, budget, and secret helpers for Anthropic-compatible fallbacks.
...
from app.utils.models.api import AccountStatus, AnthropicFallback, ProviderHealth
...
def api_key(provider: AnthropicFallbackDb) -> str:
    return crypto.decrypt(provider.api_key_enc)


def endpoint(provider: AnthropicFallbackDb, path: str) -> str:
    base = provider.base_url.rstrip("/")
    normalized_path = f"/{path.lstrip('/')}"
    # Accept both the documented API origin (https://api.anthropic.com) and
    # the common /v1-root form without ever producing /v1/v1/... URLs.
...
def is_budget_available(db: Session, provider: AnthropicFallbackDb) -> bool:
    limit = provider.monthly_spend_limit_usd
    return limit is None or monthly_spend(db, provider.id) < float(limit)


def model_ids(provider: AnthropicFallbackDb) -> Optional[Set[str]]:
...
def supports_model(provider: AnthropicFallbackDb, model: Optional[str]) -> Optional[bool]:
    if not model:
        return None
...
def normalize_expired_cooldowns(db: Session, now: Optional[datetime] = None) -> None:
    ...
    providers = db.query(AnthropicFallbackDb).filter(AnthropicFallbackDb.status == AccountStatus.COOLDOWN).all()
    changed = False
...
) -> Optional[AnthropicFallbackDb]:
    """Choose the first enabled, credential-valid, under-budget fallback."""
    normalize_expired_cooldowns(db)
    providers = (
        db.query(AnthropicFallbackDb)
        .filter(
            AnthropicFallbackDb.status == AccountStatus.ACTIVE,
            AnthropicFallbackDb.provider_health != ProviderHealth.REAUTH_REQUIRED,
        )
        .order_by(AnthropicFallbackDb.priority.asc(), AnthropicFallbackDb.created_at.asc())
        .all()
    )
    for provider in providers:
...
def mark_healthy(provider: AnthropicFallbackDb) -> None:
...
def mark_invalid(provider: AnthropicFallbackDb, message: str) -> None:
    now = datetime.now(timezone.utc)
...
def mark_cooldown(provider: AnthropicFallbackDb, seconds: int, message: str) -> None:
    now = datetime.now(timezone.utc)
...
def to_api(db: Session, provider: AnthropicFallbackDb) -> AnthropicFallback:
