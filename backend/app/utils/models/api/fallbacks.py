...
# Description: Admin API models for encrypted Anthropic-compatible fallback providers.
...
class AnthropicFallback(BaseModel):
...
class CreateAnthropicFallbackRequest(BaseModel):
...
class UpdateAnthropicFallbackRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class AnthropicFallbackResponse(BaseModel):
    fallback: AnthropicFallback


class ListAnthropicFallbacksResponse(BaseModel):
    fallbacks: List[AnthropicFallback]
