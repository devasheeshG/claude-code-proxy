...
@router.get(
    "/install.sh",
    response_class=PlainTextResponse,
    responses={200: {"description": "One-line installer for Claude Code"}},
)
def install_script() -> PlainTextResponse:
    """Installer that configures Claude Code to use this proxy and installs the status line."""
    script = r"""#!/usr/bin/env bash
set -euo pipefail

usage() {
  echo "Usage: curl -fsSL .../install.sh | bash -s -- <BASE_URL>" >&2
...
STATUSLINE_DIR="$HOME/.claude/bin"
STATUSLINE="$STATUSLINE_DIR/claude-code-statusline.sh"

umask 077
mkdir -p "$HOME/.claude" "$STATUSLINE_DIR"

SCRIPT_URL="${BASE_URL%/}/v1/clients/claude-code-statusline.sh"
echo "Downloading status line script..."
curl -fsSL "$SCRIPT_URL" -o "$STATUSLINE"
chmod +x "$STATUSLINE"

# Keep unrelated settings and write atomically. The key is supplied via a
# protected file, never as a command-line argument or generated command text.
...
if command -v python3 >/dev/null 2>&1; then
  ...
  CC_INSTALL_STATUSLINE="$STATUSLINE" \
    python3 - <<'PY'
import json
import os
import shutil
...
settings["statusLine"] = {
    "type": "command",
    "command": os.environ["CC_INSTALL_STATUSLINE"],
    "refreshInterval": 1,
}
directory = os.path.dirname(path) or "."
fd, temporary = tempfile.mkstemp(prefix="settings.json.tmp.", dir=directory)
try:
...
PY
  ...
  if [ -f "$SETTINGS" ]; then
    jq empty "$SETTINGS" || { echo "Could not parse $SETTINGS; it was left unchanged." >&2; exit 1; }
    cp -p "$SETTINGS" "$SETTINGS.bak"
  else
    printf '{}\n' > "$SETTINGS"
  fi
  TEMP_SETTINGS="$(mktemp "$HOME/.claude/settings.json.tmp.XXXXXX")"
  jq --rawfile key "$KEY_FILE" --arg base "$BASE_URL" --arg statusline "$STATUSLINE" \
    '.env = ((.env // {}) + {ANTHROPIC_BASE_URL: $base, ANTHROPIC_AUTH_TOKEN: ($key | rtrimstr("\\n"))})
    | .statusLine = {type: "command", command: $statusline, refreshInterval: 1}' \
    "$SETTINGS" > "$TEMP_SETTINGS"
  mv "$TEMP_SETTINGS" "$SETTINGS"
  TEMP_SETTINGS=""
  chmod 600 "$SETTINGS"
else
  echo "python3 or jq is required to safely update $SETTINGS." >&2
  exit 1
fi
