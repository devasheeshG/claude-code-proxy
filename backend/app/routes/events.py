...
from app.utils.postgres import AccountDb, AnthropicFallbackDb, ProxyEventDb, UsageRecordDb, get_db
...
def list_events(
    ...
    fallback_names = (
        {row.id: row.label for row in db.query(AnthropicFallbackDb).filter(AnthropicFallbackDb.id.in_(fallback_ids)).all()} if fallback_ids else {}
    )
    events = []
    for row in rows:
        metadata = {}
