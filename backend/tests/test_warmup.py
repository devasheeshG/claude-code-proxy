...
def test_pool_usage_fraction_is_aggregate_pool_capacity(seed_account):
    first = seed_account("first", session_used_pct=0.60)
    second = seed_account("second", session_used_pct=0.0)
    third = seed_account("third", session_used_pct=0.0)
    with SessionFactory() as db:
...
def test_peak_usage_can_trigger_warmup_when_average_is_diluted(seed_account):
    busy = seed_account("busy", session_used_pct=0.20)
    idle_one = seed_account("idle-one", session_used_pct=0.0)
    idle_two = seed_account("idle-two", session_used_pct=0.0)
    idle_three = seed_account("idle-three", session_used_pct=0.0)
...
def test_warmup_skips_disabled_reauthentication_and_weekly_reserve(seed_account):
    ...
    no_session_id = seed_account("no-session")
    _make_healthy(disabled_id, reserve_id, no_session_id)

    with SessionFactory() as db:
        db.get(AccountDb, reauth_id).provider_health = ProviderHealth.REAUTH_REQUIRED
        db.get(AccountDb, no_session_id).session_used_pct = None
        db.commit()
        ...
        assert warmup.is_eligible(db.get(AccountDb, no_session_id)) is False
...
def test_recent_traffic_or_warmup_means_account_is_already_warm(seed_account):
    ...
    with SessionFactory() as db:
        ...
        account.session_reset_at = now + timedelta(hours=4)
...
def test_crossing_trigger_warms_every_other_cold_account(seed_account, monkeypatch):
    active_id = seed_account("active", session_used_pct=0.60)
    cold_one_id = seed_account("cold-one", session_used_pct=0.0)
    cold_two_id = seed_account("cold-two", session_used_pct=0.0)
    _make_healthy(active_id, cold_one_id, cold_two_id)
    with SessionFactory() as db:
        active = db.get(AccountDb, active_id)
        active.session_reset_at = datetime.now(timezone.utc) + timedelta(hours=4)
