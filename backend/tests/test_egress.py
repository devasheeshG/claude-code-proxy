...
def test_proxy_target_parsing_keeps_credentials_private():
    targets = egress.parse_targets(
        ...
        '"kind":"proxy","proxy_url":"http://host.docker.internal:18181","relay_auth":true,'
        '"private_ip":"172.31.1.2","public_ip":"203.0.113.10"}]'
    )

    assert targets[0].proxy_url.endswith(":18181")
...
def test_relay_parses_authenticated_connect_request():
    ...
    assert egress_relay._target_host_allowed(host, ("api.anthropic.com", ".anthropic.com"))
    assert not egress_relay._target_host_allowed("attacker-anthropic.com", (".anthropic.com",))


def test_relay_target_config_uses_proxy_port_and_private_ip():
    targets = egress_relay._parse_targets(
        '[{"id":"egress-a","kind":"proxy","proxy_url":"http://host:18182","private_ip":"172.31.1.5","enabled":true}]'
    )

    assert targets == (egress_relay.RelayTarget("egress-a", 18182, "172.31.1.5"),)
